...
p_gam2 <- as_tibble(predict(gam_mod2, predict.data, 
                            se.fit = TRUE, type = "response")) %>%
    rename(fit_gam = fit, se_gam = se.fit)
p_gamm2 <- as_tibble(predict(gamm_mod2$gam, predict.data, 
                             se.fit = TRUE, type = "response")) %>%
    rename(fit_gamm = fit, se_gamm = se.fit)
p_gamm2AR1 <- as_tibble(predict(gamm_mod2_AR1$gam, predict.data, 
                                se.fit = TRUE, type = "response")) %>%
    rename(fit_AR1 = fit, se_AR1 = se.fit)

new_data_bases <- bind_cols(predict.data, p_gam2, p_gamm2, p_gamm2AR1) %>%
...
new_data_bases <- bind_cols(predict.data, p_tprs, p_bs) %>%
