...
```{r covariates, eval = T, fig.cap = "Relationship of select covariates to trail use for Middle Cottonwood."}
...
lm_mod <- lm(max.camera ~ max.count + 
               daily_aqi_value + 
               precipitation_in + 
               temp_max_f, 
             data = singleTrail)
...
```{r glm, eval = T, echo=T}
glm_mod <- glm(max.camera ~ max.count + 
                 daily_aqi_value + 
                 precipitation_in + 
                 temp_max_f, 
               data = singleTrail, 
...
gam_mod2 <- mgcv::gam(max.camera ~  s(yday, bs = "cc") +
                      s(month, k = 3) +
                      s(wday,
                        bs = "cc", k = 7) +
                      s(daily_aqi_value) +
...
gamm_mod <- gamm(max.camera ~  max.count + 
                 s(yday, bs = "cc") + 
                 s(wday, bs = "cc", k = 7) +
                 s(month, k = 3), 
                 data = singleTrail, 
...
gamm_mod2 <- gamm(max.camera ~ s(yday, bs = "cc") + 
                    s(month, k = 3) +
                    s(wday, 
                      bs = "cc", k = 7) +
                    s(daily_aqi_value) +
...
gamm_mod_qp <- gamm(max.camera ~ s(yday, bs = "cc") + 
                    s(month, k = 3) +
                    s(wday, 
                      bs = "cc", k = 7) +
                    s(daily_aqi_value) +
...
gamm_mod2_nb <- gamm(max.camera ~ s(yday, bs = "cc") + 
                    s(month, k = 3) +
                    s(wday, 
                      bs = "cc", k = 7) +
                    s(daily_aqi_value) +
...
gamm_mod2_gaus <- gamm(max.camera ~ s(yday, bs = "cc") + 
                    s(month, k = 3) +
                    s(wday, 
                      bs = "cc", k = 7) +
                    s(daily_aqi_value) +
...
Even with the use of `auto.arima` we must still examine diagnostic plots (in this case, ACF and PACF plots for identifying autocorrelation) and summaries for each model (see Section \@ref(diagnostic-practices) for more details).
...
After examining the summary, we should visualize our results. This may be accomplished using the `plot()` function, however we choose to use the **gratia** package [@R-gratia] and its `draw()` function to produce these plots in **ggplot2** rather than base R. 
...
new_data_bases <- bind_cols(predict.MidCot, p_gam2, p_gamm2, p_gamm2AR1) %>%
    tidyr::pivot_longer(fit_gam:se_AR1, names_sep = '_',
...
```{r gamm2 b spline}
gamm_mod2_bspline <- gamm(max.camera ~ s(yday, bs = "cc", m = c(3,1)) + 
                            s(month, k = 3) +
                            s(wday, 
                              bs = "cc", k = 7) +
                            s(daily_aqi_value, bs = "bs", m = c(3,1)) +
                            s(temp_max_f, bs = "bs", m = c(3,1)) +
                            s(precipitation_in, k = 9, bs = "bs", m = c(3,1)) +
                            max.count,
                          data = singleTrail, 
                          knots = list(yday = c(0,365)),
                          method = "REML",
