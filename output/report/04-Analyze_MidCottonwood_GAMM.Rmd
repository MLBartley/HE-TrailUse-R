...
plot(predicted ~ yday, data = pred.newdata,
     ...
     xlim = c(x.min, x.max), 
     ylim = c(0, 5000))
lines(count~ yday, data = singleTrail, type = "p", col = "black")
...
```{r gamm2 b spline}
gamm_mod2_bspline <- gamm(count ~ s(yday, bs = "bs", m = c(3,1)) + 
                            s(month, k = 3) +
                            s(wday, 
                              bs = "cc", k = 7) +
