...
high_use <- c("Baldy to Bridger", "Bridger", "Bridger to Ross Pass", 
              "College M", "M to Baldy", "Middle Cottonwood", "Sypes Canyon",
              "Ross Pass to Sacagawea Peak", "Sacagawea Pass", "Steep Way")
...
```{r modelG-predict,echo=FALSE, fig.width=6, fig.height=4, out.width="\\linewidth", fig.cap="\\label{fig:Fig}Predicted trail use count values (lines) versus observed trail use (points) for each trail subsection, based on model \\emph{G}."}
...
ggplot(data=allTrail_G, aes(x=yday, y=max.camera, group=subsectionF)) +
  facet_wrap(~subsectionF) +
  geom_ribbon(aes(ymin=(modG-2*modG_se),
                  ymax=(modG+2*modG_se)), alpha=0.25) +
  geom_line(aes(y=(modG))) +
  geom_point() +
...
high_pred_G <- ggplot(data=filter(allTrail_G, subsectionF %in% high_use),
...
low_pred_G <- ggplot(data=filter(allTrail_G, subsectionF %notin% high_use),
                   aes(x=yday, y=max.camera, group=subsectionF)) +
  facet_wrap(~subsectionF) +
  geom_ribbon(aes(ymin=(modG-2*modG_se),
                  ymax=(modG+2*modG_se)), alpha=0.25) +
...
Figure \ref{fig:modelGS-draw} shows the fitted smoothers for `gamm_modGS_ARMA_sub`. The plots of group-specific smoothers indicate that trail sections differ not only in average (log) trail use (which would correspond to each trail having a straight line at different levels for the group-level smoother), but differ slightly in the shape of their functional responses. Figure \ref{fig:Fig11} shows how the global and group-specific smoothers combine to predict uptake rates for individual plants.  We see that, unlike in the single global smoother case above, none of the curves deviate from the data systematically.

```{r modelGS-draw, fig.width=6, fig.height=3, dev.args=list(pointsize=10), out.width="\\linewidth", echo=FALSE, fig.cap="\\label{fig:Fig}Global function (\\texttt{s(yday)}) and group-specific deviations from the global function (\\texttt{s(yday, subsection)}) for \\texttt{gamm\\_modGS\\_ARMA\\_sub)}. "}
#gratia draw() plot 
gratia::draw(gamm_modGS_ARMA_sub)
```

```{r modelGS-predict,echo=FALSE, fig.width=6, fig.height=4, out.width="\\linewidth", fig.cap="\\label{fig:Fig11}Predicted trail use count values (lines) versus observed trail use (points) for each trail subsection, based on model \\emph{GS}."}
...
ggplot(data=allTrail_GS, aes(x=yday, y=max.camera, group=subsectionF)) +
  facet_wrap(~subsectionF) +
...
```{r modelGI-predict,echo=FALSE, fig.width=6, fig.height=4, out.width="\\linewidth", fig.cap="\\label{fig:}Predicted trail use count values (lines) versus observed trail use (points) for each trail subsection, based on model \\emph{GI}."}
...
ggplot(data=allTrail_GI, aes(x=yday, y=max.camera, group=subsectionF)) +
  facet_wrap(~subsectionF) +
...
-compare top version of each model type (G, GS, GI)
-compare Middle Cottonwood singleTrail to allTrail
-predict at new trails

```{r compare model type predictions}
p_G <- as_tibble(predict(gamm_modGS_ARMA_sub$gam, 
                                 predict.data, 
                                 se.fit = TRUE, 
                                 type = "response")) %>%
    rename(fit_G = fit, se_G = se.fit)
p_GS <- as_tibble(predict(gamm_modGS_sub$gam, 
                            predict.data, 
                            se.fit = TRUE, 
                            type = "response")) %>%
    rename(fit_GS = fit, se_GS = se.fit)
p_GI <- as_tibble(predict(gamm_modGI_sub$gam, 
                            predict.data, 
                            se.fit = TRUE, 
                            type = "response")) %>%
...
p_single <- as_tibble(predict(gamm_modGS_ARMA_sub$gam, 
                                 predict.data, 
                                 se.fit = TRUE, 
                                 type = "response")) %>%
...
new_data_bases <- bind_cols(predict.data, p_single, p_all) %>%
    tidyr::pivot_longer(fit_single:se_all, names_sep = '_',
                 names_to = c('variable', 'data-used')) %>%
    tidyr::pivot_wider(names_from = variable, values_from = value) %>%
    mutate(upr_ci = fit + (2 * se), lwr_ci = fit - (2 * se)) %>% 
