...
```{r subsection modG, eval = T, echo = T}
...
```{r, eval = T, echo=T}
summary(gamm_modG_sub$gam)
gratia::draw(gamm_modG_sub$gam)
gratia::appraise(gamm_modG_sub$gam)
gam.vcomp(gamm_modG_sub$gam)

rg <- gratia::rootogram(gamm_modG_sub$gam)
draw(rg)
...
with(allTrail_G, tsDiagGamm(gamm_modG_sub, timevar = yday,
                          observed = max.camera))


layout(matrix(1:2, ncol = 2))
acf(resid(gamm_modG_sub$lme, type = "normalized"), 
    lag.max = 36, main = "ACF")
pacf(resid(gamm_modG_sub$lme, type = "normalized"),
     lag.max = 36, main = "pACF")
layout(1)
```
...
```{r, eval=T, result = "asis", message=F, include=knitr::is_latex_output(), echo= T}
gamtabs(gamm_modG_sub$gam,
...
<!-- ```{=latex} -->
<!-- <table border=1> -->
<!-- <caption align="bottom"> Summaty of Global Smoothing Model (G) </caption> -->
...
<!--   <tr> <td> (Intercept) </td> <td align="right"> -8.8922 </td> <td align="right"> 1.0656 </td> <td align="right"> -8.3451 </td> <td align="right"> &lt; 0.0001 </td> </tr> -->
<!--   <tr> <td> max.count </td> <td align="right"> 0.0128 </td> <td align="right"> 0.0002 </td> <td align="right"> 56.3032 </td> <td align="right"> &lt; 0.0001 </td> </tr> -->
...
<!--   <tr> <td> s(yday) </td> <td align="right"> 8.9081 </td> <td align="right"> 8.9081 </td> <td align="right"> 3137.9257 </td> <td align="right"> &lt; 0.0001 </td> </tr> -->
<!--   <tr> <td> s(subsectionF) </td> <td align="right"> 23.9726 </td> <td align="right"> 25.0000 </td> <td align="right"> 1428.4870 </td> <td align="right"> &lt; 0.0001 </td> </tr> -->
<!--   <tr> <td> s(wday) </td> <td align="right"> 4.9718 </td> <td align="right"> 5.0000 </td> <td align="right"> 1357.9943 </td> <td align="right"> &lt; 0.0001 </td> </tr> -->
<!--   <tr> <td> s(daily_aqi_value) </td> <td align="right"> 8.8988 </td> <td align="right"> 8.8988 </td> <td align="right"> 294.5419 </td> <td align="right"> &lt; 0.0001 </td> </tr> -->
<!--   <tr> <td> s(temp_max_f) </td> <td align="right"> 8.9525 </td> <td align="right"> 8.9525 </td> <td align="right"> 121.1795 </td> <td align="right"> &lt; 0.0001 </td> </tr> -->
<!--    <a name=tab.gam></a> -->
<!-- </table> -->
<!-- ``` -->
...
allTrail_G <- transform(allTrail_G, 
                      mod_G = predict(gamm_modG_sub$gam, 
                                      type = "response"))
...
```{r subsection modG-AR1, eval = T, echo = T}
## Model G - Global smoother - A single common smoother for all observations
...
```{r}
summary(gamm_modGAR1_sub$gam)
gratia::draw(gamm_modGAR1_sub$gam)
gratia::appraise(gamm_modGAR1_sub$gam)
gam.vcomp(gamm_modGAR1_sub$gam)

rg <- gratia::rootogram(gamm_modGAR1_sub$gam)
draw(rg)

with(allTrail_G, tsDiagGamm(gamm_modGAR1_sub, timevar = yday,
                          observed = max.camera))


layout(matrix(1:2, ncol = 2))
acf(resid(gamm_modGAR1_sub$lme, type = "normalized"), 
    lag.max = 36, main = "ACF")
pacf(resid(gamm_modGAR1_sub$lme, type = "normalized"),
     lag.max = 36, main = "pACF")
layout(1)
```

```{r subsection modG-ARMA, eval = F, echo = F}
## Model G - Global smoother - A single common smoother for all observations

if(F){
  gamm_modGARA_sub <- gamm(max.camera ~
                         s(yday, bs="tp") +
                        # trailnameF + 
                         s(subsectionF,
                           bs="re",
...
gamm_modGS_sub <- gamm(max.camera ~ ## does not work with gamm; gives NA for yday:subsectionF
                         s(yday, m=2, bs="tp") +
                   s(yday, subsectionF,
                      m=2, bs="fs", k = 5) +
                     # trailnameF +
...
layout(matrix(1:2, ncol = 2))
acf(residuals(gamm_modGS_sub))
pacf(residuals(gamm_modGS_sub))
layout(1)
```
...
save(bam_modGI_sub, 
     file=here("output/models/allT_subsection_bam_GI.rda"),
     compress='xz')
...
### Accounting for Trails

### Accounting for Subsection Nesting with Trail
