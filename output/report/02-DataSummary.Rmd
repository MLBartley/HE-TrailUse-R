...
Data loading and wrangling is done with the **readxl** [@R-readxl] and **tidyverse** [@R-tidyverse] packages.

Figures in this report are made with the **ggplot2** package [@R-ggplot2] and included using the **knitr** [@R-knitr] and **kableExtra** [@R-kableExtra] packages. The color palette is derived from the **pals** package [@R-pals]. Interactive maps of trails and camera locations are created with the **mapview** package [@R-mapview]. 
...
knitr::include_graphics(here("output/figures/camera_locations.png"))
```
```{r location-test, eval= T}
...
knitr::include_graphics(path = here("output/figures/Counter_bySubsection_TS_highuse.png"))
...
For each trail counter camera deployed data is provided on a daily scale. Finer resolution data (i.e. hourly counts rather than daily) are available for 17 trails. Figures \@ref(fig:hourly-high) and \@ref(fig:hourly-low) shows trail use patterns with use hitting highest counts on the weekends, as expected. 
...
knitr::include_graphics(path = here("output/figures/hourly_bydayofweek_highuse.png"))
```
...
knitr::include_graphics(path = here("output/figures/hourly_bydayofweek_lowuse.png"))
```
...
Table \@ref(tab:strava-summary) provides a summary of Strava data for the entire year. For each trail, the aggregated annual trail use count is provided as well as the number of Strava defined edges. 
...
```
Figures \@(fig:strava-bytrailname-high) and \@ref(fig:strava-bytrailname-low) show time series plots for daily trail use counts (maximum number of trips over all edges in a trail) separated by trail (fill color indicates trail name). 
...
knitr::include_graphics(path = here("output/figures/Strava_day_TS_bytrip_high.png"))
```
...
knitr::include_graphics(path = here("output/figures/Strava_day_TS_bytrip_low.png"))
```

#### AllTrails Data

AllTrails has provided number of daily searchs for each trail for 2020-2022. A seven day preceeding moving average number of search terms was calculated for each trail. Figures \@ref(alltrails-bytrailname-high) and \@ref(alltrails-bytrailname-low) show time series plots for daily trail searches (7-day moving average) separated by trail (fill color indicates trail name). 
...
knitr::include_graphics(path = here("output/figures/allTrails_search_TS_high.png"))
```
...
knitr::include_graphics(path = here("output/figures/allTrails_search_TS_low.png"))
```
...
These data do not vary spatially only temporally (i.e. the resolution is not fine enough to parse out different weather between trails on a given day). Figure \@ref(fig:covariate-plots) shows each covariate over time for 2021. Clear collinearity between several covariates (e.g. Min and Max Air Temperature) is apparent, and is considered when selecting covariates for inclusion in analysis.
...
knitr::include_graphics(here::here("output/figures/weatherplots.png"))
