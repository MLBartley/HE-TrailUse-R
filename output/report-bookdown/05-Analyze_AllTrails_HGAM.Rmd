...
1.  Fairy Creek
2.  College M
3.  Bridger Ridge (combination of Bridger, Baldy to Bridger, Bridger to Ross Pass, Ross Pass to Sacagawea Peak)
4.  Steep Way
5.  Sacagawea Pass
6.  Carroll Creek
7.  Raptor View
...
13. Lower Shafthouse
14. Corbly Gulch - Lower
15. Corbly Gulch - Upper
16. North Cottonwood to Johnson Canyon
17. North Cottonwood Access
18. Ross Pass
19. Middle Cottonwood
20. Johnson Canyon Jeep Trail
21. Benchmark Road
22. Horsethief Mountain
23. M to Baldy
...
gratia::draw(gamm_modGI, select = c(1, 24,  4, 7, 19, 20), ncol = 2)
...
new_data_bases_OUT <- bind_cols(predict.All, p_G, p_GS, p_GI) %>%
...
compare_models <- ggplot(mapping = aes(x = yday, y = max.camera)) +
    geom_ribbon(data = new_data_bases_OUT,
                mapping = aes(ymin = lwr_ci, ymax = upr_ci, x = yday, fill = factor(model)),
                inherit.aes = FALSE, alpha = 0.2) +
    geom_point(data = allTrail, aes(colour = as.factor(wday))) +
    geom_line(data = new_data_bases_OUT, aes(y = fit, x = yday, colour2 = factor(model)),
              size = 1) %>%
    relayer::rename_geom_aes(new_aes = c("colour" = "colour2")) +
    scale_colour_brewer(palette = "Set1", aesthetics = "colour", name = "Day of Week") +
    scale_colour_OkabeIto(aesthetics = "colour2", name = "Model") +
...
compare_high <-  ggplot(mapping = aes(x = yday, y = max.camera)) +
    geom_ribbon(data = new_data_bases_OUT %>% filter(subsectionF %in% high_use),
                mapping = aes(ymin = lwr_ci, ymax = upr_ci, x = yday, fill = factor(model)),
                inherit.aes = FALSE, alpha = 0.2) +
    geom_point(data = allTrail %>% filter(subsectionF %in% high_use), aes(colour = as.factor(wday))) +
    geom_line(data = new_data_bases_OUT  %>% filter(subsectionF %in% high_use),
...
compare_low <-  ggplot(mapping = aes(x = yday, y = max.camera)) +
    geom_ribbon(data = new_data_bases_OUT %>% filter(subsectionF %notin% high_use),
                mapping = aes(ymin = lwr_ci, ymax = upr_ci, x = yday, fill = factor(model)),
                inherit.aes = FALSE, alpha = 0.2) +
    geom_point(data = allTrail %>% filter(subsectionF %notin% high_use), aes(colour = as.factor(wday))) +
    geom_line(data = new_data_bases_OUT  %>% filter(subsectionF %notin% high_use),
              aes(y = fit, x = yday, colour2 = factor(model)),
              size = 1) %>%
    relayer::rename_geom_aes(new_aes = c("colour" = "colour2")) +
    scale_colour_brewer(palette = "Set1", aesthetics = "colour", name = "Day of Week") +
...
```{r high-compare-pred2, eval = T ,fig.align='center', out.width = "100%", fig.cap="Predicted trail use count values (lines) versus observed trail use (points) for each high-use trail subsection over an entire year, based on each model (\\emph{G}, \\emph{GS}, and \\emph{GI}). Observed trip counts (data points) are colored by day of the week and show the higher use on weekends. "}

knitr::include_graphics(path =  here::here("output/figures/high_pred_compare.png"))

```

```{r low-compare-pred2, eval = T, fig.align = 'center', out.width = "100%", fig.cap="Predicted trail use count values (lines) versus observed trail use (points) for each low-use trail subsection, based on each model (\\emph{G}, \\emph{GS}, and \\emph{GI})."}
...
for(s in subsection.unique){
   ggplot(mapping = aes(x = yday, y = max.camera)) +
    geom_ribbon(data = new_data_bases_OUT %>% filter(subsectionF %in% s),
                mapping = aes(ymin = lwr_ci, ymax = upr_ci, x = yday, fill = factor(model)),
                inherit.aes = FALSE, alpha = 0.2) +
    geom_point(data = allTrail %>% filter(subsectionF %in% s), aes(colour = as.factor(wday))) +
    geom_line(data = new_data_bases_OUT  %>% filter(subsectionF %in% s),
              aes(y = fit, x = yday, colour2 = factor(model)),
              size = 1) %>%
    relayer::rename_geom_aes(new_aes = c("colour" = "colour2")) +
    scale_colour_brewer(palette = "Set1", aesthetics = "colour", name = "Day of Week") +
...
```
```{r save predictions, eval = T}
write.csv(new_data_bases_OUT, 
          file = here("output/predictions/predict_ModelG_GS_GI.csv"), 
          row.names = FALSE)
```
...
data_used <- ggplot(mapping = aes(x = yday, y = max.camera)) +
...
ggsave(filename = here::here("output/figures/MC_data_used.png"),
       plot = data_used,
       height = 8.5, 
       width = 13,
       dpi=700)
...
knitr::include_graphics(here::here("output/figures/MC_data_used.png"))
```
...
```{r predict-newtrails-prep, eval = T}
...
ggsave(filename = here::here("output/figures/NewTrails_compare.png"),
       plot = compare_models_newtrails,
       height = 8.5, 
       width = 13,
       dpi=700)
```
...
knitr::include_graphics(here::here("output/figures/NewTrails_compare.png"))
```
...
Predictions (including point estimate, standard error, and upper/lower confidence interval ranges) for models G, GS, and GI for all in-sample trail for the entire year of 2021 are available in a .csv file at `HE-TrailUse-R/output/predictions/predict_ModelG_GS_GI.csv`. It is recommended that the results for model GI be used for the current analysis predictions. 
