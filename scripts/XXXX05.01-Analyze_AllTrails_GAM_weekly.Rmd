...
gamm_modG_weekly <- gamm(max.camera ~
                         # s(yday, bs="tp", k = 30) +
                           s(week, bs = "cc") +
                         s(subsectionF,
                           bs="re",k=25) +
...
if(runall){
  ...
  gam_modG_weekly <- gam(max.camera ~
                         s(week, bs="cc") +
                        # trailnameF + 
                         s(subsectionF,
                           bs="re",
 ...
 gamm_modGAR1_weekly <- gamm(max.camera ~
                            s(week, bs="cc") +
                          # trailnameF + 
                          s(subsectionF,
                            bs="re",
                        ...
                        knots = list(week = c(1, 52)),
                          method = 'REML',
                          correlation = corAR1(form = ~week|subsectionF),
                          data = allTrail_weekly, 
                          family = poisson#, 
                          # niterPQL = 20
 )
  
 # gamm_modGARMA_weekly <- gamm(max.camera ~
 #                                s(week, bs="cc") +
 #                                # trailnameF + 
 #                                s(subsectionF,
 #                                  bs="re",
 ...
 #                              knots = list(week = c(1, 52)),
 #                              method = 'REML',
 #                              correlation = corARMA(form = ~week|subsectionF,
 #                                                  p = 1, q = 0),
 #                            data = allTrail, 
 #                            family = poisson#, 
 #                            # niterPQL = 30
...
save(gam_modG_weekly, file=here("output/models/allT_weekly_gam_G.rda"),
     compress='xz')  

  save(gamm_modG_weekly, file=here("output/models/allT_weekly_gamm_G.rda"),
     compress='xz') 
  
   save(gamm_modGAR1_weekly, file=here("output/models/allT_weekly_gamm_G-AR1.rda"),
     compress='xz')
   
#    save(gamm_modGARMA_weekly, file=here("output/models/allT_weekly_gamm_G-ARMA.rda"),
#      compress='xz')
# }

load(here("output/models/allT_weekly_gam_G.rda"))
load(here("output/models/allT_weekly_gamm_G.rda"))
load(here("output/models/allT_weekly_gamm_G-AR1.rda"))
# load(here("output/models/allT_weekly_gamm_G-ARMA.rda"))
...
arma_res_G <- forecast::auto.arima(resid(gamm_modG_weekly$lme, type = "normalized"), 
                                   seasonal = T)
...
gratia::draw(gamm_modGAR1_weekly$gam)
# gratia::appraise(gamm_modGARMA_weekly$gam)
# gam.vcomp(gamm_modGARMA_weekly$gam)
```
...
rg <- gratia::rootogram(gamm_modGAR1_weekly$gam)
draw(rg)
...
gamtabs(gamm_modGAR1_weekly$gam,
...
A. parametric coefficients & Estimate & Std. Error & t-value & p-value \\ 
  (Intercept) & 5.4505 & 0.4927 & 11.0623 & $<$ 0.0001 \\ 
  total\_traveltime & -0.0316 & 0.0145 & -2.1752 & 0.0326 \\ 
  max.count & -0.0004 & 0.0002 & -1.7764 & 0.0795 \\ 
   \hline
B. smooth terms & edf & Ref.df & F-value & p-value \\ 
  s(week) & 3.0368 & 8.0000 & 8.6139 & $<$ 0.0001 \\ 
  s(subsectionF) & 11.0691 & 14.0000 & 67.6665 & $<$ 0.0001 \\ 
  s(month) & 2.9847 & 2.9847 & 54.8160 & $<$ 0.0001 \\ 
  s(daily\_aqi\_value) & 2.3076 & 2.3076 & 15.6240 & $<$ 0.0001 \\ 
  s(temp\_max\_f) & 8.2926 & 8.2926 & 302.9206 & $<$ 0.0001 \\ 
  s(precipitation\_in) & 8.2162 & 8.2162 & 283.5341 & $<$ 0.0001 \\ 
  s(totallength\_miles) & 2.5671 & 2.5671 & 7.6129 & 0.0004 \\ 
   \hline
\end{tabular}
\caption{Summaty of Global Smoothing Model (G)} 
\label{tab.gam}
...
allTrail_G <- transform(allTrail_G, 
                      mod_G = predict(gamm_modGAR1_weekly$gam, 
                                      type = "response"))
...
gamm_modG_AR1_pred <- predict(gamm_modGAR1_weekly$gam, se.fit=TRUE, type = "response")
allTrail_G_pred <- transform(allTrail_G, 
                 modG = gamm_modG_AR1_pred$fit, 
                 modG_se = gamm_modG_AR1_pred$se.fit)
all_pred_G <- ggplot(data=allTrail_G_pred, aes(x=week, y=max.camera, group=subsectionF)) +
  ...
  labs(x=expression("Week of Year"),
...
ggsave(filename = here::here("output/figures/high_pred_modG_weekly.pdf"),
       plot = high_pred_G,
       height = 8.5, 
       width = 13,
       dpi=700)
...
ggsave(filename = here::here("output/figures/low_pred_modG_weekly.pdf"),
       plot = low_pred_G,
       height = 8.5, 
       width = 13,
       dpi=700)
...
knitr::include_graphics(path =  here::here("output/figures/high_pred_modG_weekly.pdf"))
...
knitr::include_graphics(path =  here::here("output/figures/low_pred_modG_weekly.pdf"))
```
...
```{r model GS shown, echo =T, eval =F}
 ...
 gamm_modGS_AR1_weekly <- gamm(max.camera ~ 
                           s(week, bs="cc", k = 10) +
                           s(week, subsectionF,
                             m=2, bs="fs", k = 25) +
                           s(subsectionF, bs = "re") +
...
if(runall){
  gamm_modGS_weekly <- gamm(max.camera ~ 
                            s(week, bs="cc", k = 10) +
                            s(week, subsectionF,
                              bs="fs", k = 16, xt=list(bs="cc")) +
                            s(subsectionF, bs = "re") +
                            # trailnameF +
                            s(month, k = 6) +
                            # s(wday, bs = "cc", k = 7) +
  ...
  gamm_modGS_ARMA_weekly <- gamm(max.camera ~ 
                           s(week, bs="cc", k = 10) +
                           s(week, subsectionF,
...
save(gamm_modGS_weekly, file=here("output/models/allT_weekly_gamm_GS.rda"),
     compress='xz')

save(gamm_modGS_AR1_weekly, file=here("output/models/allT_weekly_gamm_GS_AR1.rda"),
     compress='xz')

save(gamm_modGS_ARMA_weekly, file=here("output/models/allT_weekly_gamm_GS_ARMA.rda"),
     compress='xz')
}

load(here("output/models/allT_weekly_gamm_GS.rda"))
load(here("output/models/allT_weekly_gamm_GS_AR1.rda"))
# load(here("output/models/allT_weekly_gamm_GS_ARMA.rda"))

```

```{r modGS pq choice}
## this should help find values for p and q in the ARMA model 
arma_res_GS <- forecast::auto.arima(resid(gamm_modGS_weekly$lme, type = "normalized"),
                                    seasonal = T)
...
summary(gamm_modGS_ARMA_weekly$gam)
```



```{r modGS-tsDiag, eval = T}
# gratia::appraise(gamm_modGS_ARMA_weekly$gam)

with(allTrail_G, tsDiagGamm(gamm_modGS_ARMA_weekly, timevar = week,
                          observed = max.camera))

# 
# layout(matrix(1:2, ncol = 2))
# acf(residuals(gamm_modGS_ARMA_weekly$lme, type = "normalized"))
# pacf(residuals(gamm_modGS_ARMA_weekly$lme, type = "normalized"))
# layout(1)
```

Figure \ref{fig:modelGS-draw} shows the fitted smoothers for `gamm_modGS_ARMA_weekly`. The plots of group-specific smoothers indicate that trail sections differ not only in average (log) trail use (which would correspond to each trail having a straight line at different levels for the group-level smoother), but differ slightly in the shape of their functional responses. Figure \ref{fig:modelGS-predict} shows how the global and group-specific smoothers combine to predict uptake rates for individual plants.  We see that, unlike in the single global smoother case above, none of the curves deviate from the data systematically.

```{r modelGS-draw, eval= T,  fig.width=6, fig.height=3, dev.args=list(pointsize=10), out.width="\\linewidth", echo=FALSE, fig.cap="\\label{fig:Fig}Global function (\\texttt{s(yday)}) and group-specific deviations from the global function (\\texttt{s(yday, subsection)}) for \\texttt{gamm\\_modGS\\_ARMA\\_weekly)}. "}
#gratia draw() plot 
gratia::draw(gamm_modGS_ARMA_weekly)
```
...
gamm_modGS_ARMA_pred <- predict(gamm_modGS_ARMA_weekly$gam, se.fit=TRUE, type = "response")
allTrail_GS <- transform(allTrail_G, 
                 modGS = gamm_modGS_ARMA_pred$fit, 
                 modGS_se = gamm_modGS_ARMA_pred$se.fit)
all_pred_GS <- ggplot(data=allTrail_GS, aes(x=week, y=max.camera, group=subsectionF)) +
  facet_wrap(~subsectionF) +
...
high_pred_GS <- ggplot(data=filter(allTrail_GS, subsectionF %in% high_use),
                   aes(x=week, y=max.camera, group=subsectionF)) +
  facet_wrap(~subsectionF) +
  geom_ribbon(aes(ymin=(modGS-2*modGS_se),
                  ymax=(modGS+2*modGS_se)), alpha=0.25) +
  geom_line(aes(y=(modGS), col = trailname)) +
...
ggsave(filename = here::here("output/figures/high_pred_modGS_weekly.pdf"),
       plot = high_pred_GS,
       height = 8.5, 
       width = 13,
       dpi=700)
...
ggsave(filename = here::here("output/figures/low_pred_modGS_weekly.pdf"),
       plot = low_pred_GS,
       height = 8.5, 
       width = 13,
       dpi=700)
...
knitr::include_graphics(path =  here::here("output/figures/high_pred_modGS_weekly.pdf"))
...
knitr::include_graphics(path =  here::here("output/figures/low_pred_modGS_weekly.pdf"))
```
...
if(runall){
  gamm_modGI_weekly <- gamm(max.camera ~
                                  s(week,  bs="cc") +
                                  s(week, by = subsectionF,
                                    bs="cc") +
                                  s(subsectionF,
                                    bs="re",
                                    # by = trailnameF,
...
# gamm4_modGI2_weekly <- gamm4::gamm4(max.camera ~ 
#                                    s(yday, bs="tp") +
#                                    s(subsectionF, bs = "re")+
...
# )
  ...
  # gamm_modGI2_weekly <- gamm(max.camera ~ 
#                          s(yday, m=2, bs="tp") + 
#                           s(subsectionF, bs = "re") +
#                          s(yday, by = subsectionF,
#                            m=1, bs="tp") +
...
#                        family = poisson)
# 
# save(gamm_modGI_weekly, file=here("output/models/allT_weekly_gamm_GI.rda"),
#      compress='xz')
  
  gamm_modGI_AR1_weekly <- gamm(max.camera ~
                           s(week, bs="cc") +
                           s(week, by = subsectionF,
                             m=1, bs="cc") +
                           s(subsectionF,
  ...
  gamm_modGI_ARMA_weekly <- gamm(max.camera ~
                           s(week, bs="cc") +
                           s(week, by = subsectionF,
                             m=1, bs="cc") +
                           s(subsectionF,
...
save(gamm_modGI_weekly, 
     file=here("output/models/allT_weekly_gamm_GI.rda"),
     compress='xz')

save(gamm_modGI_AR1_weekly, 
     file=here("output/models/allT_weekly_gamm_GI-AR1.rda"),
     compress='xz')

save(gamm_modGI_ARMA_weekly,
     file=here("output/models/allT_weekly_gamm_GI-ARMA.rda"),
     compress='xz')
}

load(file=here("output/models/allT_weekly_gamm_GI.rda"))

load(file=here("output/models/allT_weekly_gamm_GI-AR1.rda"))

load(file=here("output/models/allT_weekly_gamm_GI-ARMA.rda"))
...
```{r modGI pq choice}
## this should help find values for p and q in the ARMA model 
arma_res_GI <- forecast::auto.arima(resid(gamm_modGI_weekly$lme, 
                                          type = "normalized"),
                      seasonal = TRUE)
...
summary(gamm_modGI_ARMA_weekly$gam)
```

```{r}
# itsadug::check_resid(gamm_modGI_AR1_weekly$gam)

gratia::draw(gamm_modGI_ARMA_weekly)
# gratia::appraise(bam_modGI_weekly)

with(allTrail_G, tsDiagGamm(gamm_modGI_ARMA_weekly, timevar = week,
                          observed = max.camera))


# layout(matrix(1:2, ncol = 2))
# acf(resid(gamm_modGI_AR1_weekly$lme, type = "normalized"), 
#     lag.max = 36, main = "ACF")
# pacf(resid(gamm_modGI_AR1_weekly$lme, type = "normalized"),
...
allTrail <- allTrail %>% 
  ...
  dplyr::ungroup()
# valRho <- acf(resid(bam_modGI_weekly), plot=FALSE)$acf[2]
# 
# 
# bam_modGI_AR1_weekly <- bam(max.camera ~ 
#                          s(yday, bs="tp") +
#                          s(subsectionF, bs = "re")+
#                          s(yday, by= subsectionF) +
#                          s(wday,
...
#                        rho=valRho)
# save(bam_modGI_AR1_weekly,
#      here("output/models/allT_weeklysection_bam_GIAR1.rda"),
#      compress='xz')

gratia::appraise(bam_modGI_AR1_weekly)

layout(matrix(1:2, ncol = 2))
acf(resid(bam_modGI_AR1_weekly), 
    lag.max = 36, main = "ACF")
pacf(resid(bam_modGI_AR1_weekly),
     lag.max = 36, main = "pACF")
layout(1)
```
...
gamm_modGI_ARMA_pred <- predict(gamm_modGI_ARMA_weekly$gam, se.fit=TRUE, type = "response")
allTrail_GI <- transform(allTrail_G, 
                 modGI = gamm_modGI_ARMA_pred$fit, 
                 modGI_se = gamm_modGI_ARMA_pred$se.fit)
all_pred_GI <- ggplot(data=allTrail_GI, aes(x=week, y=max.camera, group=subsectionF)) +
  facet_wrap(~subsectionF) +
...
p_G <- as_tibble(predict(gamm_modGAR1_weekly$gam, 
                                 predict.All_weekly, 
                                 se.fit = TRUE, 
                                 type = "response")) %>%
    rename(fit_G = fit, se_G = se.fit)
p_GS <- as_tibble(predict(gamm_modGS_AR1_weekly$gam, 
                            predict.All_weekly, 
                            se.fit = TRUE, 
                            type = "response")) %>%
    rename(fit_GS = fit, se_GS = se.fit)
p_GI <- as_tibble(predict(gamm_modGI_AR1_weekly$gam, 
                            predict.All_weekly, 
                            se.fit = TRUE, 
                            type = "response")) %>%
...
new_data_bases <- bind_cols(predict.All_weekly, p_G, p_GS, p_GI) %>%
    tidyr::pivot_longer(fit_G:se_GI, names_sep = '_',
                 names_to = c('variable', 'model')) %>%
    tidyr::pivot_wider(names_from = variable, values_from = value) %>%
    mutate(upr_ci = fit + (2 * se), lwr_ci = fit - (2 * se))

compare_models <- ggplot(mapping = aes(x = week, y = max.camera)) +
    geom_ribbon(data = new_data_bases,
                mapping = aes(ymin = lwr_ci, ymax = upr_ci, x = week, fill = factor(model)),
                inherit.aes = FALSE, alpha = 0.2) +
    geom_point(data = allTrail_weekly#,
               # aes(colour = as.factor(wday))
               ) +
    geom_line(data = new_data_bases, aes(y = fit, x = week, colour2 = factor(model)),
              size = 1) %>%
    ...
    coord_cartesian(xlim = c(0, 52), ylim = c(0, 1500)) +
...
compare_high <-  ggplot(mapping = aes(x = week, y = max.camera)) +
    geom_ribbon(data = new_data_bases %>% filter(subsectionF %in% high_use),
                mapping = aes(ymin = lwr_ci, ymax = upr_ci, x = week, fill = factor(model)),
                inherit.aes = FALSE, alpha = 0.2) +
    geom_point(data = allTrail_weekly %>% filter(subsectionF %in% high_use)) +
    geom_line(data = new_data_bases  %>% filter(subsectionF %in% high_use),
              aes(y = fit, x = week, colour2 = factor(model)),
              size = 1) %>%
    relayer::rename_geom_aes(new_aes = c("colour" = "colour2")) +
    scale_colour_brewer(palette = "Set1", aesthetics = "colour", name = "Day of Week") +
    scale_colour_OkabeIto(aesthetics = "colour2", name = "Model") +
    scale_fill_OkabeIto(name = "Model") +
    coord_cartesian(xlim = c(0, 52), ylim = c(0, 3000)) +
   facet_wrap(~subsectionF, drop = T, ncol = 3) +
    labs(title = "Extrapolation",
         subtitle = "How prediction varies with different model specifications")

ggsave(filename = here::here("output/figures/high_pred_compare_weekly.pdf"),
       plot = compare_high,
       height = 8.5, 
       width = 13,
       dpi=700)
  
compare_low <-  ggplot(mapping = aes(x = week, y = max.camera)) +
    geom_ribbon(data = new_data_bases %>% filter(subsectionF %notin% high_use),
                mapping = aes(ymin = lwr_ci, ymax = upr_ci, x = week, fill = factor(model)),
                inherit.aes = FALSE, alpha = 0.2) +
    geom_point(data = allTrail_weekly %>% filter(subsectionF %notin% high_use)) +
    ...
    scale_colour_OkabeIto(aesthetics = "colour2", name = "Model") +
    scale_fill_OkabeIto(name = "Model") +
    coord_cartesian(xlim = c(0, 52), ylim = c(0, 800)) +
   facet_wrap(~subsectionF, drop = T, ncol =3) +
    labs(title = "Extrapolation",
         subtitle = "How prediction varies with different model specifications")
...
p_all <- as_tibble(predict(gamm_modGI_AR1_weekly$gam, 
...
sapply(gamm_modGARMA_weekly$gam$smooth, "[[",  "label")
sapply(gamm_modGS_AR1_weekly$gam$smooth, "[[",  "label")
sapply(gamm_modGI_AR1_weekly$gam$smooth, "[[",  "label")

p_G_new <- as_tibble(predict(gamm_modGARMA_weekly$gam, 
                            predict.New, 
...
p_GS_new <- as_tibble(predict(gamm_modGS_AR1_weekly$gam, 
                            predict.New, 
                            exclude = c("s(subsectionF)", 
                                        "s(yday,subsectionF)"),
                            se.fit = TRUE, 
...
p_GI_new <- as_tibble(predict(gamm_modGI_AR1_weekly$gam, 
                            predict.New, 
                            exclude = c("s(subsectionF)", 
...
# AIC_table <- AIC(gamm_modGARMA_weekly$gam,
#                  gamm_modGS_ARMA_weekly$gam, 
#                  gamm_modGI_ARMA_weekly$gam)%>%
